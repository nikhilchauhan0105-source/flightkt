...
interface SelectedAirport {
  airport_code?: string; // ← was missing!
  airport_name?: string;
  airport_type?: string;
  city_name?: string;
  state_code?: string;
...
const AirportDropdown: React.FC<AirportDropdownProps> = ({ airports }) => {
  const [isOpen, setIsOpen] = useState<boolean>(false);
 const { departureSelectedAirport, setDepartureSelectedAirport } = useAirport();
  ...
  const handleSelect = (airport: SelectedAirport): void => {
    setDepartureSelectedAirport(airport);
    setIsOpen(false);
  };

  // console.log("selected FROM airport:", selectedAirport);
  ...
  return (
    ...
    >
      ...
      {/* Display selectedAirport value */}
      <div className="font-bold text-base sm:text-2xl text-gray-900 mb-0.5 w-full bg-transparent">
        {departureSelectedAirport
          ? departureSelectedAirport?.city_name
          : "Select Airport"}
      </div>

      <div className=" text-sm text-gray-500">
        {departureSelectedAirport
          ? `${departureSelectedAirport?.state_code}, ${departureSelectedAirport.airport_name?.substring(0, 35) ?? "Airport"}...`
          : "Select an airport"}
      </div>
